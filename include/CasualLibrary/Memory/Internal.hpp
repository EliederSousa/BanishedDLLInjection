...
namespace Memory {
    namespace Internal
    {
        ...
        */
        template <typename T>
        [[nodiscard]]static T read(const Address& addToBeRead, const bool memoryCheck = false) noexcept {
        ...
        */
        ...
        /**
        @brief Hot Patch bytes to NOP (90) instruction. Make sure you pass <unsigned char> to the type of the function.
        @param address address to which the value will be written to.
        ...
        */
        ...
        [[nodiscard]]static Address getModule(const char* modName) noexcept;

        /**@brief returns the dynamic pointer from base pointer \p basePrt and \p offsets*/
        [[nodiscard]]static Address getAddress(const uintptr_t basePrt, const std::vector<uintptr_t>& offsets) noexcept;

        /**@brief returns the dynamic pointer from base pointer \p basePrt and \p offsets*/
        [[nodiscard]]static Address getAddress(const Address& basePrt, const std::vector<uintptr_t>& offsets) noexcept;
        ...
        */
        [[nodiscard]]static Address findSignature(const uintptr_t start, const char* sig, const size_t size = 0) noexcept;
        [[nodiscard]]static Address findSignature(const Address& modAddr, const char* sig, const size_t size = 0) noexcept;
        ...
        @param sig Signature to search, for example: "? 39 05 F0 A2 F6 FF" where "?" (-1) is a wildcard.
        */
        [[nodiscard]]static Address findModuleSignature(const char* mod, const char* sig) noexcept;
         ...
         */
        template<typename T>
        [[nodiscard]]static T getVirtualFunction(const void* baseClass, const uint32_t index) noexcept {
        ...
        */
        ...
        static T callVirtualFunction(const void* baseClass, const uint32_t index, Args&&... args) noexcept {
